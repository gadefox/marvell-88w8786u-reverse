...
def main():
  ...
  with open(path, "rb") as file:
    while True:
      ...
      crc = crc32(header)
      if crc != 0:
        warn(f"Header: invalid CRC ({crc:X})")
        break
      ...
      if cmd == 6:
        ...
        print(Fore.BLUE + f"{seqnum} CMD6" + Style.RESET_ALL +
              ": data=" + Fore.YELLOW + f"{addr:X}({addr})" + Style.RESET_ALL +
              " crc=" + Fore.YELLOW + f"{crc:X}")
        continue
      if cmd == 7:
        if addr != length:
