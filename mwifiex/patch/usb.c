...
static const struct usb_device_id mwifiex_usb_table[] = {
	...
	/* 8786 */
	{USB_DEVICE(USB8XXX_VID, USB8786_PID_1)},
	{USB_DEVICE_AND_INTERFACE_INFO(USB8XXX_VID, USB8786_PID_2,
				       USB_CLASS_VENDOR_SPEC,
				       USB_SUBCLASS_VENDOR_SPEC, 0xff)},
	/* 8797 */
...
static int mwifiex_register_dev(struct mwifiex_adapter *adapter)
{
	...
	default:
		adapter->tx_buf_size = MWIFIEX_TX_DATA_BUF_SIZE_2K;
		strcpy(adapter->fw_name, USB8786_DEFAULT_FW_NAME);
		adapter->ext_scan = false;
		break;
	}
			...
			struct mwifiex_fw_image *fw)
{
	...
	if (card->usb_boot_state == USB8XXX_FW_DNLD) {
		...
		if (card->usb_boot_state == USB8XXX_FW_DNLD) {
			pr_err("boot state should change after successful firmware download\n");
			return -1;
		}
	}
...
MODULE_FIRMWARE(USB8786_DEFAULT_FW_NAME);
